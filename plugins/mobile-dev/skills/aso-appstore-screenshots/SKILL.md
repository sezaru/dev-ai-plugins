...
description: Generate high-converting App Store screenshots by analyzing your app's codebase, discovering core benefits, and creating ASO-optimized screenshot images using an image-capable model via OpenRouter.
user-invocable: true
---
...
Once benefits and screenshot pairings are confirmed, generate the final App Store screenshots using an image-capable model on OpenRouter (via the bundled `scripts/edit_openrouter.py` adapter — no MCP server required).

### Prerequisites Check

Generation uses an image-capable model on **OpenRouter** (via the bundled
`scripts/edit_openrouter.py` adapter — no MCP server required). Before generating,
verify:

1. `OPENROUTER_API_KEY` is set in the environment.
2. `python3` with **Pillow** is available (provided by the `dev-ai-plugins` flake's
   `mobile-dev-deps` package; otherwise `pip install Pillow`).

Check the key with `printenv OPENROUTER_API_KEY` (do NOT print the value — just confirm
it is non-empty). If it is missing, tell the user:

```
⚠️ OPENROUTER_API_KEY is not set. To generate screenshots:

1. Get a key at https://openrouter.ai/keys
2. Export it: export OPENROUTER_API_KEY=sk-or-...
3. (Optional) Pick an image model: export OPENROUTER_MODEL=<image-capable model slug>
   Default is "google/gemini-2.5-flash-image" (Nano Banana). Confirm it is available
   and image-capable on the OpenRouter models page.
4. Run this skill again.
```

The model MUST support **image input + image output** (image editing), not just
text-to-image. Do NOT proceed with generation if the key is missing.
...
**IMPORTANT — Aspect ratio mismatch**: Apple's required dimensions are narrower than standard 9:16 (~0.461 ratio vs 0.5625). Image models return their own aspect ratio, so the output is usually wider (or a different size) than Apple needs. The `edit_openrouter.py` adapter handles this automatically — after generating, it **crops (top-preserving) and resizes** each image down to the exact target dimensions. No separate `sips` step is needed.
...
2. **Stage 2 (Enhance)**: The scaffold is sent, via `scripts/edit_openrouter.py`, to an image-capable model on OpenRouter to add breakout elements, depth, and visual polish. The adapter then crops/resizes the result to exact App Store dimensions.
...
The compose.py script lives in **this skill's own directory** (the folder containing
this SKILL.md). Set `SKILL_DIR` to that absolute path. compose.py auto-detects a
heavy sans-serif font (override with `ASO_FONT=/path/to/Black.ttf` if the auto-pick
looks wrong).
...
The scaffolds are internal intermediates — do NOT show them to the user or ask for confirmation. Proceed immediately to Step 2 (OpenRouter enhancement).

**Step 2: Enhance via OpenRouter (3 versions, generated concurrently)**

Instead of an MCP tool, use the bundled adapter `scripts/edit_openrouter.py`. It fires N
requests concurrently and crops/resizes each result to the target App Store dimensions —
so a single Bash call produces all 3 final-sized versions.

For each benefit:
1. Write the enhancement prompt (from the templates below — different for the first vs
   subsequent screenshots) to a temp file, e.g. `/tmp/aso-prompt.txt`. Writing to a file
   avoids shell-escaping the long multi-line prompt.
2. Run the adapter in ONE Bash call:

```bash
SKILL_DIR="<absolute path to this skill directory>" && \
python3 "$SKILL_DIR/scripts/edit_openrouter.py" \
  --image screenshots/01-[benefit-slug]/scaffold.png \
  --prompt-file /tmp/aso-prompt.txt \
  --out-prefix screenshots/01-[benefit-slug]/v --n 3 \
  --width 1290 --height 2796
```

This writes `v1.jpg`, `v2.jpg`, `v3.jpg` — already cropped/resized to exact target
dimensions (no separate resize step). `--image` is repeatable and **order matters**:
scaffold first, then (subsequent screenshots) the style template, then (when iterating)
the approved design. Adjust `--width`/`--height` for other display sizes (see table
below). Requires `OPENROUTER_API_KEY`; model via `OPENROUTER_MODEL` or `--model`.

#### First screenshot (no approved template yet)

Pass a single `--image` (the scaffold). Use the **First screenshot prompt template** below.
...
Pass **two `--image` inputs, in this order**:
...
**IMPORTANT — Consistency enforcement**: The scaffold guarantees consistent layout. The style template guarantees consistent visual treatment. If the model changes the text, layout, or deviates from the style template, regenerate.

**Step 3: (Automatic) crop + resize**

The adapter already cropped (top-preserving — sides trimmed equally, top edge kept so the
headline stays put) and resized every version to the exact target dimensions during
Step 2. `v1.jpg`/`v2.jpg`/`v3.jpg` are App-Store-ready as written — there is **no separate
`sips` step**. For other display sizes, pass different dimensions to the adapter:
...
Present all 3 versions (`v1.jpg`, `v2.jpg`, `v3.jpg` — already at final App Store dimensions) to the user using the Read tool.

Label them clearly as **Version 1**, **Version 2**, and **Version 3** and ask the user to pick their favourite or request changes.

**Step 5: Iterate if needed**

If the user wants changes, run the adapter with **three `--image` inputs** (order matters):
...
When iterating, run the adapter again with `--n 3` (it generates 3 concurrently and resizes them). Then show the user.
...
screenshots/
  01-track-card-prices/       ← working versions for benefit 1
    ...
    v1.jpg                    ← enhanced + resized to App Store dimensions
    v2.jpg
    v3.jpg
