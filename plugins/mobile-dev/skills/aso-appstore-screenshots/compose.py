...
# Resolved at runtime by resolve_font() — a heavy/black-weight sans-serif.
FONT_PATH = None

# Platform default candidates (heavy/black sans-serif). First existing wins.
_FONT_CANDIDATES = {
    "Darwin": [
    ...
    "Linux": [
        ...
        "/usr/share/fonts/inter/Inter-Black.otf",
...
def resolve_font(explicit=None):
    """Locate a heavy sans-serif font path cross-platform.

    Order: --font/ASO_FONT -> platform defaults -> fontconfig (fc-match) -> error.
    """
    explicit = explicit or os.environ.get("ASO_FONT")
    if explicit and os.path.isfile(explicit):
        return explicit
    if explicit:
        raise SystemExit(f"Font not found: {explicit} (from --font/ASO_FONT)")

    for path in _FONT_CANDIDATES.get(platform.system(), []):
        if os.path.isfile(path):
            return path

    # fontconfig — ask for the heaviest available sans-serif.
    if shutil.which("fc-match"):
        try:
            out = subprocess.run(
                ["fc-match", "-f", "%{file}", "sans-serif:weight=black"],
    ...
    raise SystemExit(
        "No suitable font found. Set ASO_FONT=/path/to/Black.ttf (a heavy/black "
        "sans-serif) or pass --font, or install one (e.g. Inter Black)."
    )
...
def main():
    ...
    p.add_argument("--font", default=None, help="Path to a heavy sans-serif font (overrides auto-detect / ASO_FONT)")
    args = p.parse_args()
